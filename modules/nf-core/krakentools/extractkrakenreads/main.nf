process KRAKENTOOLS_EXTRACTKRAKENREADS {
    ...
    input:
    tuple val(meta), path(classified_reads_assignment), path(classified_reads_fastq), path(report), val(taxid)

    output:
    tuple val(meta), path("*.{fastq.gz,fasta.gz}"), val(taxid), emit: extracted_kraken2_reads
