...
workflow VIRUSES {
    ...
    SELECT_BEST_KMA_REF.out.chosen_ref
        .flatMap{ meta, fastas -> fastas.collect { fasta -> tuple(meta, [tag: get_species_tag_from_fasta(fasta), file: fasta]) } } // create a new job for each ref
        .set { chosen_ref_ch }

    // 3) create genome via iterative alignment/consensus calling
    // 3.1) scaffold
    BWA_INDEX_REF(chosen_ref_ch.map { meta, fasta -> [ meta, fasta, fasta.file ]})
    BWA_MEM_REF(MEGAHIT.out.contigs.combine(BWA_INDEX_REF.out.index, by: 0), "to_ref", true)
    CREATE_SCAFFOLD(BWA_MEM_REF.out.bam.join(chosen_ref_ch, by: [0, 1]), 300)
    ...
    IVAR_CONSENSUS_1(BWA_MEM_1.out.bam.join(CREATE_SCAFFOLD.out.scaffold, by: [0, 1]), "intermediate", false)
    ...
    IVAR_CONSENSUS_2(BWA_MEM_2.out.bam.join(IVAR_CONSENSUS_1.out.fasta, by: [0, 1]), "final", false)
